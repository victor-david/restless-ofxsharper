...
{
    /// <summary>
    /// Represents a collection closing statement periods.
    /// </summary>
    /// <typeparam name="T">The type which must derive from <see cref="ClosingStatementPeriod"/></typeparam>
    public class ClosingStatementPeriodCollection<T> : OfxObjectBase, ICollection<T> where T : ClosingStatementPeriod, new()
    {
        #region Private
        private List<T> list;
        #endregion
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ClosingStatementPeriodCollection{T}"/> class.
        /// </summary>
        /// <param name="rootNode">The root node from which to find data for this class.</param>
        internal ClosingStatementPeriodCollection(XmlNode rootNode, string defaultCurrency)
        {
            list = new List<T>();
            while (rootNode != null)
            {
                // construct a period. WE have to do a little gymnastics to satisfy the generics
                var period = new T();
                period.Construct(rootNode, defaultCurrency);
                Add(period);
